...
export default function EmployeeForm({ initialValues = {}, onCancel }) {
  ...
  const onFinish = async (values) => {
    try {
      ...
      if (isEditMode) {
        ...
        await employeeServices.update(initialValues._id, formData);
        toast.success("Employee updated successfully");
      } else {
        await employeeServices.create(formData);
  ...
  // Preload cascading selects based on initialValues
  // useEffect(() => {
  ...
  //       // Format profileImage for Upload preview
  ...
  //       //         status: "done",
  //       //         url:
  //       //           import.meta.env.VITE_BACKEND_URL + initialValues.profileImage,
  //       //       },
  //       //     ],
  //       //   });
  //       // }
  ...
  //       if (initialValues.profileImage) {
  //         const fileArray = Array.isArray(initialValues.profileImage)
  //           ? initialValues.profileImage.map((img) => ({
  //               uid: img._id || "-1",
  //               name: img.split("/").pop(), // get actual file name
  //               status: "done",
  //               url: import.meta.env.VITE_BACKEND_URL + img,
  //             }))
  //           : [
  //               {
  //                 uid: "-1",
  ...
  //         form.setFieldsValue({ profileImage: fileArray });
  //       }
  //     } catch (err) {
  ...
  const dynamicFields = employeesFields.map((field) => {
    switch (field.name) {
      case "country":
        return {
          ...field,
