...
export default function Drivers({ initialValues = {}, onCancel }) {
  ...
  const onFinish = async (values) => {
    try {
      ...
      if (isEditMode) {
        ...
        await driverServices.update(initialValues._id, formData);
        toast.success("Driver updated successfully");
      } else {
        await driverServices.create(formData);
      ...
      toast.error(
        isEditMode ? "Error updating driver" : "Error creating driver",
      );
    }
  };
  ...
  useEffect(() => {
    ...
    // Set all normal fields first (except profileImage)
  ...
  useEffect(() => {
    const fetchCars = async () => {
      try {
        const res = await axios.get(`${backendURL}/getCarByCompany`);
        setCars(
          res.data.map((car) => ({
            label: `${car.model}|${car.type}|${car.level}`,
            value: car._id,
          })),
        );
      } catch (err) {
  ...
  const driversFields = [
    ...
    { name: "CDL", label: "CDL", type: "text", colSpan: 12 },
    {
  ...
  const dynamicFields = driversFields.map((field) => {
    switch (field.name) {
      case "country":
        return {
          ...field,
