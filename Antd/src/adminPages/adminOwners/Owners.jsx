...
export default function Owners({ initialValues = {}, onCancel }) {
  ...
  const onFinish = async (values) => {
    try {
      ...
      if (isEditMode) {
        ...
        await ownerServices.update(initialValues._id, formData);
        toast.success("Owner updated successfully");
      } else {
        await ownerServices.create(formData);
      ...
      queryClient.invalidateQueries(["owners"]); // refresh the employee list
    } catch (error) {
      ...
      toast.error(isEditMode ? "Error updating owner" : "Error creating owner");
    }
  };
  ...
  const dynamicFields = ownersFields.map((field) => {
    switch (field.name) {
      case "country":
        return {
          ...field,
